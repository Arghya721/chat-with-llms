...
def calculate_cost(input_string, output_string, model_name):
    ...
    if (
        chat_config["company"] == "Mistral"
        or chat_config["company"] == "Perplexity"
        or chat_config["company"] == "Meta"
    ):
...
async def custom_http_exception_handler(request, exc: HTTPException):
    ...
    return JSONResponse(
        ...
        content={
            "status": exc.status_code if exc.status_code else status.HTTP_403_FORBIDDEN,
            "details": exc.detail,
        },
    )
...
def update_generations_left(
    token_info: dict = Depends(verify_google_token), generations_left: int = 30
):
...
async def chat_event_streaming(
    ...
    try:
        ...
        def event_streaming():
            ...
            try:
                ...
                input_token_length, output_token_length, cost = calculate_cost(
...
@app.get(
    "/v1/chat_history", tags=["AI Endpoints"], response_model=list[ChatUserHistory]
)
async def user_chat_history(
    ...
    try:
        ...
        chat_ref = (
            db.collection("chats")
            .where("google_user_id", "==", token_info["sub"])
        ...
        for chat_data in chat_ref:
            ...
            chat_history.append(
                ChatUserHistory(
                    ...
                    chat_title=chat_data.get("chat_title", None),
                    chat_model=chat_data.get("model", "gpt-3.5-turbo"),
                )
            )
        ...
        if chat_data:
            ...
            if chat_data["google_user_id"] == token_info["sub"]:
                ...
                for chat_data in chat_history_ref:
                    ...
                    chat_history.append(
                        ChatByIdHistory(
                            ai_message=chat_data["ai_message"],
                            user_message=chat_data["user_message"],
                            created_at=chat_data["created_at"],
                            updated_at=chat_data["updated_at"],
                            regenerate_message=chat_data["regenerate_message"],
...
async def fetch_receipt(token_info: dict = Depends(verify_token)):
    ...
    try:
        ...
        receipt_ref = (
            db.collection("payments")
            .where("customer_id", "==", token_info["sub"])
            .order_by("updated_at", direction=google_firestore.Query.DESCENDING)
            .stream()
        )
